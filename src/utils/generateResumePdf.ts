export async function generateResumePdf(info: any, pdfMake: any) {
    // Helper function to create sections in the Bronzor style
    function createSection(title: string, contentElements: any[] | object) {
        return {
            table: {
                widths: [85, '*'], // Width for section title column, '*' for content
                body: [
                    [
    ...
    const experienceContent = info.experience.flatMap((exp: any) => {
        const content: any[] = [
            {
                columns: [
    ...
    const educationContent = info.education.map((edu: any) => ({
        stack: [
            {
                columns: [
    ...
    for (const categoryKey in info.skills) {
        ...
        skillsContent.push({ text: (info.skills as any)[categoryKey].join(', '), style: 'skillItemsCv', margin: [0, 0, 0, 5] });
    }

    // Content for Certifications Section
    const certificationsContent = info.certifications.map((cert: any) => ({
        stack: [
            {
                columns: [
    ...
    const languagesContent = info.languages.map((lang: any) => ({
        columns: [
    ...
    // Contact info line
    const contactInfoLine = `${info.location}  •  ${info.contact.phone}  •  ${info.contact.email}  •  ${info.contact.website}`;
    ...
    pdfMake.createPdf(docDefinition).download(`${info.name.replace(/\s+/g, '_')}-Resume_BronzorStyle.pdf`);
}